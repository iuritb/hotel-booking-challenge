...
import { u as useFetch } from './fetch-D66RcAZs.mjs';
import 'nanoid';
import '../_/nitro.mjs';
import 'node:http';
import 'node:https';
  ...
  setup(__props) {
    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      _push(ssrRenderComponent(_component_NuxtLink, mergeProps({
        to: `/hotels/${__props.hotel.id}`,
        class: "block bg-white rounded-lg shadow-md hover:shadow-lg transition-shadow duration-300 overflow-hidden"
      }, _attrs), {
        default: withCtx((_, _push2, _parent2, _scopeId) => {
          if (_push2) {
            _push2(`<img${ssrRenderAttr("src", __props.hotel.imageUrl)}${ssrRenderAttr("alt", __props.hotel.name)} class="w-full h-48 object-cover"${_scopeId}><div class="p-4"${_scopeId}><h3 class="text-xl font-semibold text-gray-800 mb-2"${_scopeId}>${ssrInterpolate(__props.hotel.name)}</h3><p class="text-gray-600 text-sm mb-2"${_scopeId}>${ssrInterpolate(__props.hotel.location)}</p><div class="flex items-center mb-2"${_scopeId}><div class="flex text-yellow-500"${_scopeId}><!--[-->`);
            ssrRenderList(5, (n) => {
              _push2(`<!--[-->`);
              if (n <= Math.floor(__props.hotel.rating)) {
                _push2(ssrRenderComponent(_component_Icon, { name: "ic:round-star" }, null, _parent2, _scopeId));
              } else if (n - 0.5 === __props.hotel.rating) {
            ...
            _push2(`<!--]--></div><span class="ml-2 text-gray-700 text-sm"${_scopeId}>${ssrInterpolate(__props.hotel.rating?.toFixed(1) || "N/A")} (${ssrInterpolate(__props.hotel.reviewsCount)} avaliações)</span></div><p class="text-gray-900 text-lg font-bold mb-4"${_scopeId}>R$ ${ssrInterpolate(__props.hotel.pricePerNight?.toFixed(2) || "0.00")} / noite</p><button class="${ssrRenderClass([
            ...
            if (isCompared.value) {
              ...
              _push2(` Adicionar à Comparação <!--]-->`);
            }
            _push2(`</button></div>`);
          } else {
            return [
              createVNode("img", {
                src: __props.hotel.imageUrl,
                alt: __props.hotel.name,
                class: "w-full h-48 object-cover"
              }, null, 8, ["src", "alt"]),
              createVNode("div", { class: "p-4" }, [
                createVNode("h3", { class: "text-xl font-semibold text-gray-800 mb-2" }, toDisplayString(__props.hotel.name), 1),
                createVNode("p", { class: "text-gray-600 text-sm mb-2" }, toDisplayString(__props.hotel.location), 1),
                createVNode("div", { class: "flex items-center mb-2" }, [
                  createVNode("div", { class: "flex text-yellow-500" }, [
                    (openBlock(), createBlock(Fragment, null, renderList(5, (n) => {
                      return openBlock(), createBlock(Fragment, { key: n }, [
                        n <= Math.floor(__props.hotel.rating) ? (openBlock(), createBlock(_component_Icon, {
                          key: 0,
                          name: "ic:round-star"
                        })) : n - 0.5 === __props.hotel.rating ? (openBlock(), createBlock(_component_Icon, {
                          key: 1,
                          name: "ic:round-star-half",
                          class: "h-5 w-5"
                  ...
                  createVNode("span", { class: "ml-2 text-gray-700 text-sm" }, toDisplayString(__props.hotel.rating?.toFixed(1) || "N/A") + " (" + toDisplayString(__props.hotel.reviewsCount) + " avaliações)", 1)
                ]),
                createVNode("p", { class: "text-gray-900 text-lg font-bold mb-4" }, "R$ " + toDisplayString(__props.hotel.pricePerNight?.toFixed(2) || "0.00") + " / noite", 1),
                createVNode("button", {
                  ...
                  isCompared.value ? (openBlock(), createBlock(Fragment, { key: 0 }, [
                    ...
                    createTextVNode(" Na Comparação ")
                  ], 64)) : (openBlock(), createBlock(Fragment, { key: 1 }, [
                    createVNode(_component_Icon, {
                      name: "ic:round-add",
                      class: "inline-block align-middle mr-1"
                    }),
                    createTextVNode(" Adicionar à Comparação ")
                  ], 64))
                ], 2)
              ])
            ];
...
const __nuxt_component_2 = Object.assign(_sfc_main$1, { __name: "HotelCard" });
  ...
  async setup(__props) {
    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      if (unref(authStore).isAuthenticated) {
        _push(`<p class="text-lg text-gray-700 mb-8"> Bem-vindo de volta, <span class="font-semibold">${ssrInterpolate(unref(authStore).currentUser?.name || "Usuário")}</span>! Vamos encontrar sua próxima estadia? </p>`);
      } else {
        _push(`<p class="text-lg text-gray-700 mb-8"> Faça login ou cadastre-se para aproveitar todas as nossas funcionalidades de reserva! </p>`);
      }
      _push(`<div class="bg-white p-6 rounded-lg shadow-md mb-8"><h2 class="text-2xl font-semibold text-gray-900 mb-4"> Buscar e Filtrar Hotéis </h2><form class="grid grid-cols-1 md:grid-cols-6 gap-4 items-end"><div class="md:col-span-2">`);
      ...
      _push(`</div><div class="md:col-span-3 mt-4"><label for="sort-by" class="block text-sm font-medium text-gray-700 mb-1">Ordenar por</label><select id="sort-by" class="mt-1 block w-full py-2 px-3 border border-gray-300 bg-white rounded-md shadow-sm focus:outline-none focus:ring-blue-500 focus:border-blue-500 sm:text-sm"><option value=""${ssrIncludeBooleanAttr(Array.isArray(searchCriteria.value.sortBy) ? ssrLooseContain(searchCriteria.value.sortBy, "") : ssrLooseEqual(searchCriteria.value.sortBy, "")) ? " selected" : ""}>Nenhum</option><option value="pricePerNight"${ssrIncludeBooleanAttr(Array.isArray(searchCriteria.value.sortBy) ? ssrLooseContain(searchCriteria.value.sortBy, "pricePerNight") : ssrLooseEqual(searchCriteria.value.sortBy, "pricePerNight")) ? " selected" : ""}>Preço</option><option value="rating"${ssrIncludeBooleanAttr(Array.isArray(searchCriteria.value.sortBy) ? ssrLooseContain(searchCriteria.value.sortBy, "rating") : ssrLooseEqual(searchCriteria.value.sortBy, "rating")) ? " selected" : ""}>Avaliação</option></select></div><div class="md:col-span-3 mt-4"><label for="sort-order" class="block text-sm font-medium text-gray-700 mb-1">Ordem</label><select id="sort-order" class="mt-1 block w-full py-2 px-3 border border-gray-300 bg-white rounded-md shadow-sm focus:outline-none focus:ring-blue-500 focus:border-blue-500 sm:text-sm"><option value="asc"${ssrIncludeBooleanAttr(Array.isArray(searchCriteria.value.sortOrder) ? ssrLooseContain(searchCriteria.value.sortOrder, "asc") : ssrLooseEqual(searchCriteria.value.sortOrder, "asc")) ? " selected" : ""}>Crescente</option><option value="desc"${ssrIncludeBooleanAttr(Array.isArray(searchCriteria.value.sortOrder) ? ssrLooseContain(searchCriteria.value.sortOrder, "desc") : ssrLooseEqual(searchCriteria.value.sortOrder, "desc")) ? " selected" : ""}>Decrescente</option></select></div><div class="md:col-span-2 hidden">`);
      _push(ssrRenderComponent(_component_BaseInput, {
        id: "check-in-date",
      ...
      _push(`</div></form></div>`);
      if (unref(pending)) {
        _push(`<div class="text-center text-lg text-gray-600"> Carregando hotéis... </div>`);
      } else if (unref(error)) {
        _push(`<div class="text-center text-red-600 text-lg"> Erro ao carregar hotéis: ${ssrInterpolate(unref(error).message || "Verifique sua conexão.")}</div>`);
...
//# sourceMappingURL=index-DeDWFB2j.mjs.map
