...
const _sfc_main = /* @__PURE__ */ defineComponent({
  ...
  async setup(__props) {
    ...
    const { data: response, pending, error } = ([__temp, __restore] = withAsyncContext(() => useFetch(`/api/hotels/${hotelId}`, "$CiB1mQtzyJ")), __temp = await __temp, __restore(), __temp);
    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      if (unref(pending)) {
        ...
        _push(`<!--]--><span class="ml-2 text-gray-700 text-sm">${ssrInterpolate(hotel.value.rating?.toFixed(1) || "N/A")} (${ssrInterpolate(hotel.value.reviewsCount)} avaliações) </span></div><h2 class="text-2xl font-semibold text-gray-800 mt-6 mb-3">Descrição</h2><p class="text-gray-700 leading-relaxed mb-6">${ssrInterpolate(hotel.value.description)}</p><h2 class="text-2xl font-semibold text-gray-800 mt-6 mb-3">Comodidades</h2><ul class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-2 mb-6"><!--[-->`);
        ...
        _push(`<!--]--></ul><h2 class="text-2xl font-semibold text-gray-800 mt-6 mb-3">Disponibilidade e Preço</h2><p class="text-gray-700 mb-4">Quartos disponíveis: <span class="font-bold">${ssrInterpolate(hotel.value.availableRooms)}</span></p><p class="text-3xl font-bold text-indigo-600 mb-6">R$ ${ssrInterpolate(hotel.value.pricePerNight?.toFixed(2) || "0.00")} <span class="text-lg text-gray-500">/ noite</span></p></div><div class="lg:w-1/3 bg-gray-50 p-4 md:p-6 rounded-lg shadow-inner mt-8 lg:mt-0"><h2 class="text-2xl font-semibold text-gray-800 mb-4">Reservar este Hotel</h2><p class="text-gray-700 mb-6"> Para prosseguir com a reserva de ${ssrInterpolate(hotel.value.name)}, clique no botão abaixo. Você será direcionado para um formulário de reserva detalhado. </p>`);
        if (unref(authStore).isAuthenticated) {
          ...
          _push(`<div class="text-center text-gray-700"><p class="mb-2">Você precisa estar logado para fazer uma reserva.</p>`);
        ...
        _push(`<div${ssrRenderAttrs(mergeProps({ class: "min-h-screen flex items-center justify-center" }, _attrs))}><p class="text-gray-600">Hotel não encontrado.</p></div>`);
      }
    };
  }
});
...
//# sourceMappingURL=index-CP9C9w8M.mjs.map
